 ...
 */
package com.example.jiangyue.androidap.chart.tools;
    ...
    public void apply(int zoom_axis) {
        if (mChart instanceof XYChart) {
            ...
            for (int i = 0; i < scales; i++) {
                ...
                //取缩放最大值
                if (mRenderer.getmZoomInMaxX() != 0 && mRenderer.getmZoomInMaxY() != 0) {
                    newWidth = Math.min(newWidth, mRenderer.getmZoomInMaxX());
                    newHeight = Math.min(newHeight, mRenderer.getmZoomInMaxY());
                }
                //取缩放最小值
                ...
                if (mRenderer.isZoomXEnabled() && (zoom_axis == ZOOM_AXIS_X || zoom_axis == ZOOM_AXIS_XY)) {
                    ...
                    if (mRenderer.getmZoomInMaxX() != 0 && mRenderer.getmZoomInMaxY() != 0) {
                        newXMin = newXMax > mRenderer.getmZoomInMaxX() ? newXMin : mRenderer.getmZoomStartPosX();
                        newXMax = newXMax > mRenderer.getmZoomInMaxX() ? mRenderer.getmZoomEndPosX() : newXMax;
                    }
                    setXRange(newXMin, newXMax, i);
                    mRenderer.setmZoomStartPosX(newXMin);
                    mRenderer.setmZoomEndPosX(newXMax);
                }
                if (mRenderer.isZoomYEnabled() && (zoom_axis == ZOOM_AXIS_Y || zoom_axis == ZOOM_AXIS_XY)) {
