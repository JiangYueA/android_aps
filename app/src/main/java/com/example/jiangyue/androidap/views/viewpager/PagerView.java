...
public class PagerView extends ViewPager {
    ...
    private LoopTransformer transform;// 页面滑动监听
    private OnPageChangeListener mScrollListener;
    private boolean scrolled = true; //头尾是否可以滑动
    ...
    /* 页面滑动监听 */
    public interface LoopTransformer {
        public void transformPage(int index, View view, float position);
    }

    public void setTransformPage(LoopTransformer transform) {
    ...
    public void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (transform != null) {
            for (int i = 0; i < childCount; i++) {
                ...
                final PagerView.LayoutParams lp = (PagerView.LayoutParams) child.getLayoutParams();
                if (lp.isDecor) continue;
                float transformPos = (float) (child.getLeft() - l) / (getMeasuredWidth() - getPaddingLeft() - getPaddingRight());
                transform.transformPage(i, child, transformPos);
            }
        }
    }
